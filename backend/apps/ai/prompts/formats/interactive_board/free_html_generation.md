...
- **Orientierung (nicht starre Regeln):**
  ...
  - **Phaser (3.x):** **Szenen**, klare **Update-/Zeitschritt-Logik**, Eingaben, optionale Arcade-Physik — gut, wenn der Ablauf **wie eine kleine Simulation oder gestaffelte Szene** gedacht ist (auch ohne „Spielstand“ oder Punkte). **Kein** Phaser-4-Code.
...
- **`phaser` → `window.Phaser`** (optional): **ausschließlich Phaser 3.80.x** (lokales `phaser.min.js`) — **Szenen** (`Phaser.Scene`), **Game-Loop**, Sprites, Eingaben; **nicht nur „Spiele“**: auch **didaktische Szenenfolgen**, **Simulationen**, viele gleichartige Objekte. **`phaser` in `used_libraries`**, sobald `Phaser.` im Code vorkommt. **Verboten:** Phaser-4-Syntax, Tutorials für „v4“, sowie Fantasiemethoden (z. B. `setStrokeStyle` an `fillStyle`-Rückgaben). **`Graphics`:** `fillStyle` / `lineStyle` (o. ä.) wie in **Phaser-3-Doku** — **kein** Blind-Ketten, wenn du dir unsicher bist: **getrennte Aufrufe** auf dem **selben** `Graphics`-Objekt. **`Phaser.Geom.RoundedRectangle`** nur in der **Phaser-3**-Form (`new Phaser.Geom.RoundedRectangle(...)`), **kein** Mix mit Canvas-`createRadialGradient` auf dem Phaser-WebGL-Hauptcanvas ohne eigenes 2D-Canvas. **Höchstens eine** schwere Szenen-Engine (**Phaser** / **Pixi** / größeres **Konva**) pro Board.
