...
export function WorksheetEditSidebar({
  ...
  if (isCreativeHtmlWorksheetContent(draft)) {
    const cPages = (draft.pages as { page_label?: string; html?: string; page_css?: string }[]) || [];
    ...
    const setPageCss = (pageIndex: number, page_css: string) => {
      setDraft((prev) => {
        const next = JSON.parse(JSON.stringify(prev)) as Record<string, unknown>;
        const pg = (next.pages as Record<string, unknown>[])[pageIndex];
        if (pg) (next.pages as Record<string, unknown>[])[pageIndex] = { ...pg, page_css };
        return next;
      });
    };
            ...
            return (
              <section key={pageIndex} className="card p-4 shadow-sm">
                ...
                <div className="mt-5 flex flex-wrap items-center gap-3 border-t border-[var(--color-border)] pt-4">
                  <label className="sr-only" htmlFor={`cr-ins-kind-${pageIndex}`}>
                    Typ für neuen Abschnitt
                  </label>
                  <select
                    id={`cr-ins-kind-${pageIndex}`}
                    className="select h-8 max-w-[12rem] px-2 text-[12px]"
                    value={insertKind[pageIndex] ?? 'text'}
                    onChange={(e) =>
                  ...
                  <button
                    ...
                    onClick={() =>
                      applyCreativeSnippets(pageIndex, [
                        ...snippets,
                        defaultCreativeFlowSnippetForKind(insertKind[pageIndex] ?? 'text'),
                      ])
                    }
                  >
                    + Block unten
                  ...
                  {onRegenerateWorksheetPage ? (
                    <button
                      type="button"
                      className={cn(
                        'btn btn-secondary btn-sm inline-flex items-center gap-1',
                        kiPanelOpen[pageIndex] && 'border-indigo-400 bg-indigo-50',
                      )}
                ...
                <details className="mt-3 rounded-lg border border-slate-200 bg-slate-50/80 px-2 py-2 text-[11px]">
                  <summary className="cursor-pointer font-medium text-slate-700">Seiten-CSS (optional)</summary>
                  <div className="mt-3 space-y-3">
                    <div>
                      <p className="text-[10px] text-slate-600">Nur nötig, wenn du Styles manuell anpassen willst.</p>
                      <label htmlFor={`ws-cr-css-${pageIndex}`} className="field-label mt-2 text-[11px]">
                        CSS (nur diese Seite)
                      </label>
                      <textarea
                        id={`ws-cr-css-${pageIndex}`}
                        className="input mt-1 min-h-[72px] font-mono text-[10px] leading-snug"
                        value={String(page.page_css ?? '')}
                        onChange={(e) => setPageCss(pageIndex, e.target.value)}
                        spellCheck={false}
                      />
                    </div>
                  </div>
