...
import {
  worksheetsLibraryCommentsQueryKey,
  worksheetLibraryEntryQueryKey,
} from '../../../lib/listQueries';
import { cn } from '../../../lib/cn';
import {
  fetchWorksheetLibraryComments,
  postWorksheetLibraryComment,
} from '../worksheetsApi';
import { backofficeDeleteWorksheetLibraryComment } from '../../boards/boardsApi';

const COMMENT_MAX = 2000;

export function WorksheetLibraryCommentsSection({
  worksheetId,
  commentCount,
  ...
  const q = useQuery({
    queryKey: worksheetsLibraryCommentsQueryKey(worksheetId),
    queryFn: () => fetchWorksheetLibraryComments(worksheetId),
    enabled: loadComments,
    staleTime: 20_000,
  });

  const postMut = useMutation({
    mutationFn: (text: string) => postWorksheetLibraryComment(worksheetId, text),
    onSuccess: () => {
      setDraft('');
      void queryClient.invalidateQueries({ queryKey: worksheetsLibraryCommentsQueryKey(worksheetId) });
      void queryClient.invalidateQueries({ queryKey: worksheetLibraryEntryQueryKey(worksheetId) });
      void queryClient.invalidateQueries({ queryKey: ['worksheets', 'library'] });
    },
  });

  const deleteMut = useMutation({
    mutationFn: (commentId: string) => backofficeDeleteWorksheetLibraryComment(worksheetId, commentId),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: worksheetsLibraryCommentsQueryKey(worksheetId) });
      void queryClient.invalidateQueries({ queryKey: worksheetLibraryEntryQueryKey(worksheetId) });
      void queryClient.invalidateQueries({ queryKey: ['worksheets', 'library'] });
  ...
  return (
    ...
    >
      ...
      <p className="mb-3 text-[11px] leading-relaxed text-[var(--color-ink-500)]">
        Alle Beiträge erscheinen als{' '}
        <span className="font-medium text-[var(--color-ink-700)]">Anonym</span> — keine Namen oder Kontaktdaten
        sichtbar.
        {staffModeration ? (
      ...
      {loadComments ? (
        ...
        <p className="mb-4 text-sm text-[var(--color-ink-500)]">
          Kommentare werden geladen, wenn der Eintrag sichtbar wird …
        </p>
      )}

      <Field
        label="Kommentar hinzufügen"
        htmlFor={`worksheet-lib-comment-${worksheetId}`}
        help={`Max. ${COMMENT_MAX} Zeichen.`}
      >
        <textarea
          id={`worksheet-lib-comment-${worksheetId}`}
          className={cn(
